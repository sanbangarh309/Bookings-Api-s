...
if (isset($_REQUEST['action']) && $_REQUEST['action'] =='add_offer') {
	...
	if (isset($_REQUEST['activity_type']) && !empty($_REQUEST['activity_type'])) {
		foreach ($_REQUEST['activity_type'] as $key => $value) {
			if ($value && $value !='') {
				$newarra[strtolower($value)]['offer_value'] = $_REQUEST['offer_value'][$key];
				$newarra[strtolower($value)]['dress_code'] = $_REQUEST['dress_code'][$key];
				$newarra[strtolower($value)]['details'] = $_REQUEST['details'][$key];
				$newarra[strtolower($value)]['start_time'] = $_REQUEST['start_time'][$key];
				$newarra[strtolower($value)]['end_time'] = $_REQUEST['end_time'][$key];
			}
		}
	}
	$curdate=strtotime($_REQUEST['start_date']);
	$start = new MongoDate(strtotime($_REQUEST['start_date']));
	$tm = $_REQUEST['start_time'][0];
	$st = $_REQUEST['start_date'];
	$findate = date('Y-m-d H:i:s', strtotime("$st $tm"));
	// echo '<pre>';print_r($_REQUEST['start_date']);exit;
	...
	if(strtotime($_REQUEST['start_date']) >= strtotime(date("Y-m-d")))
	{
		...
		if (isset($_REQUEST['business_id']) && $_REQUEST['business_id'] !='' && isset($_REQUEST['start_date']) && $_REQUEST['start_date'] !='') {
			...
			$data = array('business_id' => $_REQUEST['business_id'], 'name' => ucfirst($_REQUEST['name']),'venue_id'=>$_REQUEST['venue_id'],'start_date' => $findate, 'type' => 'offer','distance'=>0,'lat'=>0,'lng'=>0,'reservation_count'=>0,'checkins_count'=>0,'status'=>$status,'day_limit'=>$_REQUEST['day_limit'],'selected_weeks'=>json_encode($_REQUEST['selected_weeks']),'week_limit'=>$_REQUEST['week_limit'],'available_hrs'=>$_REQUEST['available_hrs'],'cncl_hrs'=>$_REQUEST['cncl_hrs'],'activity_type'=>$activities,'spots'=>$_REQUEST['spots'],'category'=>$_REQUEST['category'],'created_date'=>$created_date);
			try {
