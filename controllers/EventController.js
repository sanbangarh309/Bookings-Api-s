...
/*************/
router.post('/check_upload', function (req, res) {
...
router.post('/add_event',sanUpload, function (req, res) {
  var path = 'https://'+req.headers.host+'/files/events/';
  ...
  var hours = sanban.sanGetDurations(today,req.body.start_date);
 ...
 for (var name in req.body.activity_type) {
  if (req.body.activity_type[name].length > 0) {
    if (req.body.offer_value) {
      newarra.push({offer_value:req.body.offer_value[name]});
    }
    if (req.body.dress_code) {
      newarra.push({dress_code:req.body.dress_code[name]});
    }
    if (req.body.details) {
      newarra.push({details:req.body.details[name]});
    }
    if (req.body.start_time) {
      newarra.push({start_time:req.body.start_time[name]});
    }
    if (req.body.end_time) {
      newarra.push({end_time:req.body.end_time[name]});
    }
  }
}
  ...
  /* Save Method */
  var offer = new Event();
  offer.business_id = req.body.business_id;
  offer.name = req.body.name;
  offer.venue_id = req.body.venue_id;
  offer.start_date = new Date(req.body.start_date);
  offer.type = 'offer';
  offer.image = newpath;
  offer.distance = 0;
  offer.lat = 0;
  ...
  offer.reservation_id = 0;
  offer.checkins_count = 0;
  offer.time = req.body.time;
  offer.week_limit = req.body.week_limit;
  offer.available_hrs = req.body.available_hrs;
  offer.cncl_hrs = req.body.cncl_hrs;
  offer.spots = req.body.spots;
  offer.category = req.body.category;
  offer.selected_weeks = JSON.stringify(req.body.selected_weeks);
  offer.activity_type = newarra;
  offer.status = 'AVAILABLE SOON';
   // res.status(200).send(events);
   offer.save(function(err) {
     if (err) return res.status(500).send({ status: false,detail: 'error'});
     return res.redirect('/admin/offers?add=1');
   });
   /***************/
 });
// Venue.remove({}, function(err) {
...
router.post('/add_venue',sanUpload3, function (req, res) {
  var path = 'https://'+req.headers.host+'/files/venues/';
...
router.get('/detail', function (req, res) {
  var id = req.query.event_id || req.query.venue_id;
  var modelid = req.query.model_id;
  // var venue_id = req.query.venue_id;
  sanban.sanGetEventByVenue(req, res, id, modelid, function(events) {
    res.status(200).send(events);
  });
});
...
router.post('/add_reserve',qrcode_image, async (req, res)=> {
  ...
  if (model_data.block == 1 || model_data.status == 0) {
    return res.status(200).send({'msg':'Model is blocked Or Not Approved!'});
  }else{
        ...
        if (bangar == 3 || sannnn == 1) {
          if (bangar == 3) {
            sanban.blockmodel(req, res, function(models){
              return res.status(200).send({result : 'you are blocked!. Can use Maximum 3 offers per day'});
            })
          }else if (sannnn == 1) {
            return res.status(200).send({result : 'Event already Booked!'});
          }
        }else{
          var day_limit = 0;
          var week_limit = 0;
          ...
          if (events.type == 'offer' && stop !=1) {
            ...
            try {
              var selected_weeks = JSON.parse(events.selected_weeks);
            } catch (e) {
              var selected_weeks = events.selected_weeks;
            }
          ...
          var isafter = moment(events.start_date).isAfter(new Date());
          if (isafter && selected_weeks.week.includes(req.body.day)) {
            var timings = selected_weeks.timings[req.body.day];
            if (timings.start) {
              var beginningTime = moment(timings.start.toLowerCase(), 'h:mma');
            }
            if (req.body.time) {
              var endTime = moment(req.body.time.toLowerCase(), 'h:mma');
            }
            if (beginningTime && endTime && !beginningTime.isBefore(endTime)) {
              return res.status(200).send({result : 'Selected Time Not Available'});
                // console.log(endTime.toDate());
              }
            }else{
              return res.status(200).send({result : 'Selected Day Not Available'});
            }
          // res.json(selected_weeks);
          //process.exit();
          var newpath = '';
          if (events.status =='AVAILABLE' && stop !=1 && events.spots > 0) {
            if ((day_limit < events.day_limit && dates.indexOf(today.getTime()) != '-1') && events.type =='offer' && activity.length > 0) {
              var path = 'https://'+req.headers.host+'/files/qrcodes/';
              Reserve.create({
              ...
              function (err, reservation) {
                ...
                // ,spots: -1
                Event.findByIdAndUpdate(reservation.event_id, { $inc: { reservation_count: 1},$set: { "booking_model_ids": idsss}}, function(err, event){
                  if (event) {
                    sanban.sanGetEventById(req,res,event._id, function(events){
                      sanban.sanGenerateQRCode(req,res,reservation._id, function(image){
                        ...
                        event.spots = parseInt(event.spots)-1;
                        event.save();
                        res.status(200).send(events);
                      });
                    });
              ...
              if (cancel ==1) {
                Reserve.findByIdAndUpdate(reserv_id, { $set: { "status": 'init'}}, function(err, reservation){
                  ...
                  Event.findByIdAndUpdate(reservation.event_id, { $inc: { reservation_count: 1,spots: -1 },$set: { "booking_model_ids": idsss}}, function(err, event){
                    if (event) {
                      sanban.sanGetEventById(req,res,event._id, function(events){
                        sanban.sanGenerateQRCode(req,res,reservation._id, function(image){
                          events.reservation_id = reservation._id;
